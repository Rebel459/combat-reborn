...
public class QuiverItem extends Item {
    ...
    public static float getFullnessDisplay(ItemStack itemStack) {
        QuiverContents quiverContents = itemStack.getOrDefault(CRDataComponents.QUIVER_CONTENTS.get(), QuiverContents.empty(QuiverHelper.getType(itemStack)));
        return quiverContents.weight().floatValue();
    }

    @Override
    public boolean overrideStackedOnOther(ItemStack itemStack, Slot slot, ClickAction clickAction, Player player) {
        QuiverContents quiverContents = itemStack.get(CRDataComponents.QUIVER_CONTENTS.get());
        if (quiverContents == null) {
            ...
            if (clickAction == ClickAction.PRIMARY && !itemStack2.isEmpty()) {
                ...
                itemStack.set(CRDataComponents.QUIVER_CONTENTS.get(), mutable.toImmutable());
    ...
    public Optional<TooltipComponent> getTooltipImage(ItemStack quiverStack) {
        ...
        if (!display.shows(CRDataComponents.QUIVER_CONTENTS.get())) {
            return Optional.empty();
        }

        QuiverContents contents = quiverStack.get(CRDataComponents.QUIVER_CONTENTS.get());
